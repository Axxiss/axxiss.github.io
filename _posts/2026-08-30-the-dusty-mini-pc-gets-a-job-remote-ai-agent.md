...
I'm not starting from scratch, my setup evolved over the years, I'm currently starting from a semi-autonomous setup were I'm using Conductor to manage and control the agents and their worktrees. But as I wanted to run the agent on a devcontainer for a contained blast radius I switched to Paseo.

In a nutshell, my current setup goes like this:
...
Power is a side effect of parallelism. When agents do their work they require energy to run tests, create artefacts, do lots of IO, etc; which if unplugged means draining the laptop's battery. One day it's fine, do it every day and the battery will notice it. So you're forced to be plugged-in all day, beating the point of having a laptop. Furthermore, it's annoying, the laptop gets heated up, the fan is constantly working. Your fingertips feel it.
